...
import com.alps.oauth.server.mapper.BaseActionMapper;
...
public class BaseActionServiceImpl extends BaseServiceImpl<BaseActionMapper, BaseAction> implements ActionService {
    @Autowired
    ...
    public BaseAction addAction(BaseAction aciton) {
        if (isExist(aciton.getActionCode())) {
            throw new AlpsAlertException(String.format("%s编码已存在!", aciton.getActionCode()));
        }
        ...
        // 同步权限表里的信息
        authorityService.saveOrUpdateAuthority(aciton.getActionId(), ResourceType.action);
        return aciton;
    }
    ...
    public BaseAction updateAction(BaseAction aciton) {
        ...
        if (saved == null) {
            throw new AlpsAlertException(String.format("%s信息不存在", aciton.getActionId()));
        }
    ...
    public void removeAction(Long actionId) {
        ...
        authorityService.removeAuthority(actionId, ResourceType.action);
