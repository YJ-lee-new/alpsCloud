...
import com.alps.system.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService
{
     ...
     */
    @Override
    public List<Ztree> roleMenuTreeData2(SysRole role)
    {
        Long roleId = role.getRoleId();
        List<Ztree> ztrees = new ArrayList<Ztree>();
        List<SysMenu> menuList = menuMapper.selectMenuAll();
        ...
        {
            List<String> roleMenuList = menuMapper.selectMenuTree(roleId);
            ztrees = initZtree2(menuList, roleMenuList, true);
        }
        return ztrees;
    }
    ...
    public List<Ztree> initZtree2(List<SysMenu> menuList, List<String> roleMenuList, boolean permsFlag)
    {
        List<Ztree> ztrees = new ArrayList<Ztree>();
        boolean isCheck = StringUtils.isNotNull(roleMenuList);
        for (SysMenu menu : menuList)
        {
            ...
            ztree.setTitle(menu.getMenuName());
            if (isCheck && !"F".equals(menu.getMenuType())&&roleMenuList.contains(menu.getMenuId() + menu.getPerms()))
            {
                ztree.setChecked(roleMenuList.contains(menu.getMenuId() + menu.getPerms()));
                ztrees.add(ztree);
            }
