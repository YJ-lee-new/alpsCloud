...
import com.alps.provider.system.domain.SysRoleDept;
import com.alps.provider.system.domain.SysRoleMenu;
import com.alps.provider.system.domain.SysUserRole;
import com.alps.system.mapper.SysRoleDeptMapper;
import com.alps.system.mapper.SysRoleMapper;
import com.alps.system.mapper.SysRoleMenuMapper;
import com.alps.system.mapper.SysUserRoleMapper;
import com.alps.system.service.ISysRoleService;
...
public class SysRoleServiceImpl implements ISysRoleService
{
	...
	/* (non-Javadoc)
	 * @see com.alps.system.service.ISysRoleService#updateGateway(com.alps.provider.system.domain.SysRole)
	 */
	@Override
	public int updateGateway(SysRole role) {
		PfActionRef pfActionRef = new PfActionRef();
		pfActionRef.setRoleId(role.getRoleId());
		pfActionRef.setMenuId(role.getMenuId());
		pfActionRefMapper.deletePfActionRefByAc(pfActionRef);
    ...
    public int insertRoleGateway(SysRole role)
    {
        int rows = 1;
        // 新增用户与角色管理
        List<PfActionRef> list = new ArrayList<PfActionRef>();
        for (String actionId : role.getActions())
        {
        	PfActionRef rm = new PfActionRef();
            rm.setId(GetUUID.getGUID());
            rm.setCompanyId("HP");
            rm.setRoleId(role.getRoleId());
            rm.setMenuId(role.getMenuId());
        ...
        if (list.size() > 0)
        {
            rows = pfActionRefMapper.batchRoleGateway(list);
        }
        return rows;
    }
}
